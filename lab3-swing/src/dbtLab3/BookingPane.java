...
public class BookingPane extends BasicPane {
	...
	private void fillNameList() {
		...
		LinkedList<String> list = (LinkedList<String>) db.getMovies();
		for(int i = 0; i < list.size(); i++){
			nameListModel.addElement(list.get(i));
		}
	}
	...
	private void fillDateList() {
		...
		LinkedList<String> list = (LinkedList<String>) db.getDates(nameList.getSelectedValue());
		for(int i = 0; i < list.size(); i++){
			dateListModel.addElement(list.get(i));
		}
	}
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			String seats = db.getSeats(movieName, date, theater);
			fields[MOVIE_NAME].setText(movieName);
			fields[PERF_DATE].setText(date);
			fields[THEATER_NAME].setText(theater);
			fields[FREE_SEATS].setText(seats);
		}
	}
